...
def get_traces(from_block, to_block, from_address):
...
def main(input_file, start_block):
    from_address = input_file.read().splitlines()
    ...
    with Progress() as progress:
        ...
        for a, b in block_ranges(start_block, end_block, step):
            trace_tasks.append(client.submit(get_traces, a, b, from_address))
        ...
        for futures, traces in as_completed(trace_tasks, with_results=True):
            receipt_tasks.append(
                client.submit(
                    get_receipts, [trace["transactionHash"] for trace in traces]
                )
            )
    ...
    costs = df.groupby("sender").sum().sort_values("gas_cost", ascending=False).gas_cost
    costs.to_csv(f"{prefix}-gas-costs.csv")
    print(costs)
